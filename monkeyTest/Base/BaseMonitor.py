...
def get_battery(devices):
    try:
        # 使用参数列表提高安全性
        cmd = ["adb", "-s", devices, "shell", "dumpsys", "battery"]
        print(" ".join(cmd))  # 打印完整的命令字符串
        output = subprocess.check_output(cmd).decode()  # 直接解码输出
        ...
        if match:
            ...
            raise ValueError("Unable to find battery level in output")
    ...
    # 写入电池电量信息到文件
    file_path = PATH(f"../info/{devices}_battery.pickle")
    writeInfo(battery_level, file_path)
...
def get_pid(pkg_name, devices):
    ...
    cmd = "adb -s " + devices + " shell ps | grep " + pkg_name
    print("----get_pid-------")
    print(cmd)
...
def get_flow(pid, type, devices):
    upflow = downflow = 0
    if pid is not None:
        cmd = ["adb", "-s", devices, "shell", "cat", f"/proc/{pid}/net/dev"]
        print(" ".join(cmd))  # 打印完整的命令字符串
        try:
            _flow = subprocess.Popen(cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE).stdout.readlines()
            for item in _flow:
                print(item.split()[0])
                if type == "wifi" and item.split()[0] == b'wlan0:':  # wifi
                    upflow = int(item.split()[1])
                    downflow = int(item.split()[9])
                    print("------flow---------")
                    print(upflow)
                    break
                elif type == "gprs" and item.split()[0] == "rmnet0:":  # gprs
                    upflow = int(item.split()[1])
                    downflow = int(item.split()[9])
                    print("-----flow---------")
                    print(upflow)
    ...
    # 写入流量信息到文件
    file_path = PATH(f"../info/{devices}_flow.pickle")
    writeFlowInfo(upflow, downflow, file_path)
...
def get_cpu_kel(devices):
    ...
    try:
        ...
        # 假设每行 "processor" 表示一个 CPU 核心
        processor_count = len(re.findall(r'^processor', output, re.MULTILINE))
