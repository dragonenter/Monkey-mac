...
def check_device_connection(device):
    try:
        # 尝试执行一个简单的ADB命令来检查设备是否连接
        subprocess.check_output(f"adb -s {device} shell echo", shell=True)
        return True
...
def start(devicess):
    ...
    # if not os.path.exists(mc["monkey_log"]):
    #     with open(mc["monkey_log"], 'w', encoding='utf-8')  as monkeylog:
    #         pass  # 创建文件，不需要写入任何内容
    try:
        start_monkey("adb -s " + devices + " shell " + mc["cmd"], mc["log"])
    except Exception as e:
        print(f"发生异常：{e}")
        if not check_device_connection(devices):
            print(f"Monkey测试在设备 {devices} 上异常退出。")
    ...
    while True:
        try:
            with open(mc["monkey_log"], 'r',encoding='utf-8') as monkeylog:
                time.sleep(1)  # 每1秒采集检查一次
                if not check_device_connection(devices):
                    print(f"设备 {devices} 已断开连接，测试中断。")
                    break
                ...
                if monkeylog.read().count('Monkey finished')  > 0 or (not check_device_connection(devices)):
